...
/**
 * Simple notes manager for the desktop NotesWidget.
 * Each item is an object with "id", "content", "attachments" (array of file paths), "createdAt".
 * Deliberately stored separately from the overlay's Notes.json (Directories.notesPath) so the
 * two note-taking surfaces don't clobber each other's data.
 */
Singleton {
    id: root
