...
Scope {
    ...
    readonly property var apiKeys: AiPolicy.onlineAllowed ? (KeyringStorage.keyringData?.apiKeys ?? {}) : ({})
    readonly property bool apiKeysLoaded: AiPolicy.onlineAllowed ? KeyringStorage.loaded : true
    ...
    property var models: AiPolicy.onlineAllowed ? {
    ...
    property var apiStrategies: {
        ...
        if (AiPolicy.onlineAllowed) {
            result.gemini = geminiApiStrategy.createObject(root);
            result.mistral = mistralApiStrategy.createObject(root);
        }
        return result;
    }
    ...
    function setModel(modelId, feedback = true, setPersistentState = true) {
        ...
        if (modelList.indexOf(modelId) !== -1) {
            ...
            if (AiPolicy.localOnly && !model.endpoint.includes("localhost")) {
                root.addMessage(
    ...
    function setApiKey(key) {
        if (!AiPolicy.onlineAllowed) {
            root.addMessage(Translation.tr("Online API keys are disabled by the AI policy"), root.interfaceRole);
            return;
        }
        const model = models[currentModelId];
        if (!model.requires_key) {
        ...
        root.addMessage(Translation.tr("API key set for %1").arg(model.name), root.interfaceRole);
    }
    ...
    function printApiKey() {
        ...
        if (model.requires_key) {
            ...
            if (key) {
                root.addMessage(Translation.tr("API key:\n\n```txt\n%1\n```").arg(key), root.interfaceRole);
            } else {
                root.addMessage(Translation.tr("No API key set for %1").arg(model.name), root.interfaceRole);
            }
        } else {
            root.addMessage(Translation.tr("%1 does not require an API key").arg(model.name), root.interfaceRole);
        }
    ...
    function shutdown() {
        ...
        getUserPrompts.running = false;
        getSavedChats.running = false;
        requester.running = false;
        decodeImageAndAttachProc.running = false;
