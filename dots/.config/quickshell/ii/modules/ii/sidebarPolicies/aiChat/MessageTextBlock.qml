        ...
        model: ScriptModel {
            ...
            onValuesChanged: {
                while (textLinesRepeater.textLineOpacities.length < values.length) {
                    textLinesRepeater.textLineOpacities.push((root.messageData?.done ?? false) ? 1 : 0);
                }
            }
        }
        delegate: TextArea {
            ...
            // Fade in animation
            visible: opacity > 0
            opacity: fadeChunkSplitting ? (textLinesRepeater.textLineOpacities[index] ?? ((root.messageData?.done ?? false) ? 1 : 0)) : 1
            Connections {
                target: root.messageData ?? null
                function onDoneChanged() {
                    if (root.messageData?.done ?? false) {
