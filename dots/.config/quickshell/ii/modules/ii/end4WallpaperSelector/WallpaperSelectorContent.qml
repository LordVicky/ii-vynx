...
MouseArea {
    id: root
    property int columns: Config.options.end4WallpaperSelector.columns || 4
    property real previewCellAspectRatio: 4 / 3
    property bool useDarkMode: Appearance.m3colors.darkmode
    property bool showControls: false
    property string source: "local"
    property string selectedResolution: "1080p"
    property bool toolbarVisible: showControls || Config.options.end4WallpaperSelector.showSearchbar
    property bool filterFieldFocused: false

    property var quickDirs: [
        { icon: "home",       name: "Home   ",       path: `${Directories.home}`,                alwaysVisible: Config.options.end4WallpaperSelector.showHomePath },
        { icon: "wallpaper",  name: "Wallpapers   ", path: `${Directories.pictures}/Wallpapers`, alwaysVisible: true },
        { icon: "imagesmode", name: "Homework   ",   path: `${Directories.pictures}/homework`,   alwaysVisible: (Config.options.policies.weeb ?? 0) === 1 },
        { icon: "casino",     name: "Random   ",     path: `${Directories.pictures}/Random`,     alwaysVisible: true },
        {
            icon: "image",
            name: Config.options.end4WallpaperSelector.userPath?.trim().length > 0
                ? Config.options.end4WallpaperSelector.userPath.split("/").filter(s => s.length > 0).pop() + "   "
                : "Custom   ",
            path: Config.options.end4WallpaperSelector.userPath,
            alwaysVisible: Config.options.end4WallpaperSelector.userPath?.trim().length > 0
        }
    ]
    ...
    Keys.onPressed: event => {
        if (event.key === Qt.Key_Escape) {
            ...
            if (Config.options.end4WallpaperSelector.showSearchbar) {
                Config.options.end4WallpaperSelector.showSearchbar = false
                showControls = false
            } else {
    ...
    Rectangle {
        ...
        Item {
            ...
            Rectangle {
                ...
                visible: !Config.options.end4WallpaperSelector.showBlurBackground
            }
            ...
            ColumnLayout {
                ...
                Item {
                    ...
                    RowLayout {
                        ...
                        StyledComboBox {
                            ...
                            implicitWidth: 120
                            model: [
                                { value: "local",     displayName: Translation.tr("Local"),     icon: "" },
                                { value: "wallhaven", displayName: Translation.tr("Wallhaven"), icon: "" },
                                { value: "unsplash",  displayName: Translation.tr("Unsplash"),  icon: "" },
                                { value: "pexels",    displayName: Translation.tr("Pexels"),    icon: "" },
                            ]
                ...
                Item {
                    ...
                    RowLayout {
                            ...
                            sourceComponent: Toolbar {
                                IconToolbarButton {
                                    ...
                                    altAction: () => {
                                        Wallpapers.openFallbackPicker(root.useDarkMode);
                                        GlobalStates.end4WallpaperSelectorOpen = false;
                                        Config.options.end4WallpaperSelector.useSystemFileDialog = true;
                                    }
                                    text: "open_in_new"
                                    StyledToolTip {
                                ...
                                ToolbarTextField {
                                    ...
                                    Connections {
                                        ...
                                        function onEnd4WallpaperSelectorOpenChanged() {
                                            if (!GlobalStates.end4WallpaperSelectorOpen) onlineSearchField.text = ""
                                        }
                                    }
                                    Keys.onPressed: event => {
                                        if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter) {
                        ...
                        ToolbarPairedFab {
                            iconText: "close"
                            onClicked: GlobalStates.end4WallpaperSelectorOpen = false
                        }
                    }
                }
            }
    ...
    Connections {
        target: GlobalStates
        function onEnd4WallpaperSelectorOpenChanged() {
            if (GlobalStates.end4WallpaperSelectorOpen && monitorIsFocused) {
    ...
    Connections {
        target: Wallpapers
        function onChanged() {
            if (Config.options.end4WallpaperSelector.closeAfterSelection)
                GlobalStates.end4WallpaperSelectorOpen = false;
        }
    }
}
