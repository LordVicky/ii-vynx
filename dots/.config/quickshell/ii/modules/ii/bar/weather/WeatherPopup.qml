...
StyledPopup {
    ...
    readonly property var forecastData: effectiveContentItem?.forecastData ?? []
    readonly property bool forecastLoading: effectiveContentItem?.forecastLoading ?? true
    readonly property var filteredHourlyData: effectiveContentItem?.filteredHourlyData ?? []

    function getDayName(dateStr, index) {
        return effectiveContentItem?.getDayName(dateStr, index) ?? "";
    }

    function formatHour(timeStr) {
        return effectiveContentItem?.formatHour(timeStr) ?? "";
    }

    function getHourlyTempRange() {
        return effectiveContentItem?.getHourlyTempRange() ?? {
            min: 0,
            max: 100
        };
    }
    ...
    lazyContent: Component {
        ColumnLayout {
            ...
            function fetchForecast() {
                ...
                let requestedCity = city || "auto";
                requestedCity = requestedCity.trim().split(/\s+/).join('+');
                forecastFetcher.command[2] = `curl -s "wttr.in/${requestedCity}?format=j1" | jq '{daily: [.weather[] | {date: .date, maxC: .maxtempC, minC: .mintempC, maxF: .maxtempF, minF: .mintempF, code: .hourly[4].weatherCode}], hourly: [.weather[0].hourly[], .weather[1].hourly[] | {time: .time, tempC: .tempC, tempF: .tempF, code: .weatherCode}]}'`;
            ...
            Process {
                ...
                stdout: StdioCollector {
                    onStreamFinished: {
                        if (text.length === 0) {
                            contentLayout.forecastLoading = false;
                            return;
                        }
                        try {
                            const data = JSON.parse(text);
                            contentLayout.forecastData = data.daily || [];
                            contentLayout.hourlyData = data.hourly || [];
                        } catch (e) {
