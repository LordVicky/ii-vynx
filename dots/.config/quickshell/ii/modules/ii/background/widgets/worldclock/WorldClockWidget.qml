...
AbstractBackgroundWidget {
    ...
    // Snap a dragged width to the nearest of the two layout modes, mirroring
    // CalendarWidget.modeForWidth().
    function modeForWidth(value) {
        return value < (276 + 420) / 2 ? "2x2" : "4x1"
    }
    ...
    // Note: keyboard-focus suppression for the actual timezone search fields is
    // handled inside StyledComboBoxSearch itself (tied to its TextField's focus),
    // not here - showingSettings alone doesn't mean the user is typing.
    ...
    Item {
        ...
        Rectangle {
            ...
            WidgetResizeHandle {
                ...
                onRequestSize: (newWidth, newHeight) => {
                    const newMode = root.modeForWidth(newWidth)
                    if (newMode === root.sizeMode) return
                    if (root.showingSettings) root.showingSettings = false
                    root.sizeMode = newMode
                }
                onRequestSizeCommit: root.configEntry.sizeMode = root.sizeMode
            }
        }
    }
}
