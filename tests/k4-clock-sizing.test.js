...
test("clock hover uses measured asymmetric sequential zones", () => {
    ...
    assert.match(view, /id:\s*clockText[\s\S]*?anchors\.left:\s*leftDate\.right[\s\S]*?anchors\.leftMargin:\s*root\.zoneGap/);
    assert.match(view, /id:\s*rightIndicators[\s\S]*?anchors\.left:\s*clockText\.right[\s\S]*?anchors\.leftMargin:\s*root\.zoneGap/);
    assert.doesNotMatch(view, /anchors\.horizontalCenter:\s*parent\.horizontalCenter/);
    assert.doesNotMatch(view, /anchors\.right:\s*parent\.right/);
    ...
    assert.match(builtins, /islandWidth:\s*44 \+ leftWidth \+ zoneGap \+ centerWidth \+ zoneGap \+ rightWidth/);
