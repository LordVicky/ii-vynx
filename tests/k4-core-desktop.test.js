...
test("core built-ins preserve k4 priorities, activation defaults and dimensions", () => {
    ...
    assert.match(source, /name:\s*"player"[\s\S]*?priority:\s*55[\s\S]*?active:\s*enabled[\s\S]*?IslandState\.hovered && root\.passiveHoverAllowed[\s\S]*?K4Media\.hasPlayer[\s\S]*?K4Media\.isPlaying \|\| root\.playerHoverSession[\s\S]*?trackPeekOpen[\s\S]*?islandWidth:\s*340[\s\S]*?K4Media\.hasTimeline \? 140 : 115/);
