...
test("K4 host resolves Away-when-fullscreen per monitor and only Reserve claims space", async () => {
    ...
    assert.match(source, /exclusiveZone:\s*panelWindow\.effectiveSpaceMode === "reserve"\s*\?\s*K4Theme\.baseHeight \+ panelWindow\.shapeInset\s*:\s*0/);
...
test("K4 Hidden withdraws only the island drawing and keeps a narrow reveal strip", async () => {
    ...
    assert.match(source, /id:\s*withdrawTranslate[\s\S]*?panelWindow\.withdrawn[\s\S]*?panelWindow\.bottom[\s\S]*?island\.height \+ panelWindow\.shapeInset \+ 6[\s\S]*?: -\(island\.height \+ panelWindow\.shapeInset \+ 6\)[\s\S]*?duration:\s*360[\s\S]*?Easing\.OutCubic/);
