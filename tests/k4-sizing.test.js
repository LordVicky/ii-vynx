...
test("k4 width scale changes horizontal geometry only", () => {
    const bar = read("modules/ii/k4bar/K4Bar.qml");

    assert.match(bar, /readonly property real widthScale:\s*K4Settings\.widthScale/);
    assert.match(bar, /readonly property int islandBodyWidth:\s*Math\.round\([\s\S]*?desiredBodyWidth : pluginVisible\.islandWidth\)[\s\S]*?\* widthScale\)/);
    assert.match(bar, /bottomHoverBridge[\s\S]*?idleContent\.desiredBodyWidth \* panelWindow\.widthScale[\s\S]*?K4Theme\.wing \* 2/);
