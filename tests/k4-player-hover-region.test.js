...
test("player hover is armed only from the scaled idle media region and clears on island exit", () => {
    ...
    const playerBlock = builtins.match(/property QtObject playerPlugin:[\s\S]*?property QtObject toastPlugin:/)?.[0] ?? "";
    assert.match(playerBlock, /active:\s*enabled\s*&&\s*\([\s\S]*?IslandState\.hovered\s*&&\s*K4Media\.hasPlayer\s*&&\s*root\.playerHoverSession/);
    assert.doesNotMatch(playerBlock, /root\.passiveHoverAllowed/);
