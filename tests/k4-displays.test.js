...
test("K4 Displays does not duplicate dynamic workspace ownership", async () => {
    const plugin = await read("modules/ii/k4bar/K4DisplaysPlugin.qml");
    const view = await read("modules/ii/k4bar/K4DisplaysView.qml");

    assert.doesNotMatch(plugin, /HyprlandData\.workspaces|updateWorkspaces\(|workspacerules|workspaceAssignments|workspaceStatement|workspaceMoveStatement|workspace_rule|workspace\.move/);
    assert.doesNotMatch(view, /Workspace routing|setWorkspaceAssignment|label:\s*"Workspaces"/);
});
