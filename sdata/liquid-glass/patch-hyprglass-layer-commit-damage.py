...
def main() -> int:
    ...
    render_anchor = """static void hkRenderLayer(Render::IHyprRenderer* thisptr, PHLLS layerSurface, PHLMONITOR monitor,\n                           const Time::steady_tp& now, bool popups, bool lockscreen) {\n    const auto& config = g_pGlobalState->config;\n"""
    render_block = """static void hkRenderLayer(Render::IHyprRenderer* thisptr, PHLLS layerSurface, PHLMONITOR monitor,\n                           const Time::steady_tp& now, bool popups, bool lockscreen) {\n    if (layerSurface && layerSurface->m_namespace == \"quickshell:bar-glass\") {\n        const auto position = layerSurface->position(Desktop::View::IGeometric::GEOMETRIC_CURRENT);\n        const auto size = layerSurface->size(Desktop::View::IGeometric::GEOMETRIC_CURRENT);\n        const auto bufferSize = layerSurface->m_layerSurface && layerSurface->m_layerSurface->m_surface\n            ? layerSurface->m_layerSurface->m_surface->m_current.size\n            : Vector2D{};\n        std::ostringstream line;\n        line << \"render-enter x=\" << position.x << \" y=\" << position.y\n             << \" w=\" << size.x << \" h=\" << size.y\n             << \" bufferW=\" << bufferSize.x << \" bufferH=\" << bufferSize.y\n             << \" popups=\" << popups\n             << \" snapshot=\" << g_pHyprRenderer->m_bRenderingSnapshot;\n        traceLayerCommit(line.str());\n    }\n\n    const auto& config = g_pGlobalState->config;\n"""
